const SURAH_DATA = {
  number: 77,
  name: "Al-Mursalat",
  nameArabic: "الْمُرْسَلَات",
  meaning: "Those Sent Forth",
  totalAyahs: 50,
  juz: 30,
  audioBaseUrl: "https://everyayah.com/data/Alafasy_128kbps/",
  ayahs: [
    {
      number: 1,
      arabic: "وَالْمُرْسَلَاتِ عُرْفًا",
      translation: "By those [winds] sent forth in gusts",
      words: [
        { arabic: "وَالْمُرْسَلَاتِ", english: "By those sent forth" },
        { arabic: "عُرْفًا", english: "in gusts" }
      ]
    },
    {
      number: 2,
      arabic: "فَالْعَاصِفَاتِ عَصْفًا",
      translation: "And the winds that blow violently",
      words: [
        { arabic: "فَالْعَاصِفَاتِ", english: "And the winds that blow" },
        { arabic: "عَصْفًا", english: "violently" }
      ]
    },
    {
      number: 3,
      arabic: "وَالنَّاشِرَاتِ نَشْرًا",
      translation: "And [by] the winds that spread [clouds]",
      words: [
        { arabic: "وَالنَّاشِرَاتِ", english: "And the winds that spread" },
        { arabic: "نَشْرًا", english: "clouds" }
      ]
    },
    {
      number: 4,
      arabic: "فَالْفَارِقَاتِ فَرْقًا",
      translation: "And those [angels] who bring criterion",
      words: [
        { arabic: "فَالْفَارِقَاتِ", english: "And those who bring" },
        { arabic: "فَرْقًا", english: "criterion" }
      ]
    },
    {
      number: 5,
      arabic: "فَالْمُلْقِيَاتِ ذِكْرًا",
      translation: "And those [angels] who deliver a message",
      words: [
        { arabic: "فَالْمُلْقِيَاتِ", english: "And those who deliver" },
        { arabic: "ذِكْرًا", english: "a message" }
      ]
    },
    {
      number: 6,
      arabic: "عُذْرًا أَوْ نُذْرًا",
      translation: "As justification or warning",
      words: [
        { arabic: "عُذْرًا", english: "As justification" },
        { arabic: "أَوْ", english: "or" },
        { arabic: "نُذْرًا", english: "warning" }
      ]
    },
    {
      number: 7,
      arabic: "إِنَّمَا تُوعَدُونَ لَوَاقِعٌ",
      translation: "Indeed, what you are promised is to occur",
      words: [
        { arabic: "إِنَّمَا", english: "Indeed, what" },
        { arabic: "تُوعَدُونَ", english: "you are promised" },
        { arabic: "لَوَاقِعٌ", english: "is to occur" }
      ]
    },
    {
      number: 8,
      arabic: "فَإِذَا النُّجُومُ طُمِسَتْ",
      translation: "So when the stars are obliterated",
      words: [
        { arabic: "فَإِذَا", english: "So when" },
        { arabic: "النُّجُومُ", english: "the stars" },
        { arabic: "طُمِسَتْ", english: "are obliterated" }
      ]
    },
    {
      number: 9,
      arabic: "وَإِذَا السَّمَاءُ فُرِجَتْ",
      translation: "And when the heaven is opened",
      words: [
        { arabic: "وَإِذَا", english: "And when" },
        { arabic: "السَّمَاءُ", english: "the heaven" },
        { arabic: "فُرِجَتْ", english: "is opened" }
      ]
    },
    {
      number: 10,
      arabic: "وَإِذَا الْجِبَالُ نُسِفَتْ",
      translation: "And when the mountains are blown away",
      words: [
        { arabic: "وَإِذَا", english: "And when" },
        { arabic: "الْجِبَالُ", english: "the mountains" },
        { arabic: "نُسِفَتْ", english: "are blown away" }
      ]
    },
    {
      number: 11,
      arabic: "وَإِذَا الرُّسُلُ أُقِّتَتْ",
      translation: "And when the messengers' time has come",
      words: [
        { arabic: "وَإِذَا", english: "And when" },
        { arabic: "الرُّسُلُ", english: "the messengers'" },
        { arabic: "أُقِّتَتْ", english: "time has come" }
      ]
    },
    {
      number: 12,
      arabic: "لِأَیِّ يَوْمٍ أُجِّلَتْ",
      translation: "For what Day was it postponed?",
      words: [
        { arabic: "لِأَیِّ", english: "For what" },
        { arabic: "يَوْمٍ", english: "Day" },
        { arabic: "أُجِّلَتْ", english: "was it postponed" }
      ]
    },
    {
      number: 13,
      arabic: "لِيَوْمِ الْفَصْلِ",
      translation: "For the Day of Judgement",
      words: [
        { arabic: "لِيَوْمِ", english: "For the Day" },
        { arabic: "الْفَصْلِ", english: "of Judgement" }
      ]
    },
    {
      number: 14,
      arabic: "وَمَا أَدْرَاكَ مَا يَوْمُ الْفَصْلِ",
      translation: "And what can make you know what is the Day of Judgement?",
      words: [
        { arabic: "وَمَا", english: "And what" },
        { arabic: "أَدْرَاكَ", english: "can make you know" },
        { arabic: "مَا", english: "what is" },
        { arabic: "يَوْمُ", english: "the Day" },
        { arabic: "الْفَصْلِ", english: "of Judgement" }
      ]
    },
    {
      number: 15,
      arabic: "وَيْلٌ يَوْمَئِذٍ لِّلْمُكَذِّبِينَ",
      translation: "Woe, that Day, to the deniers",
      words: [
        { arabic: "وَيْلٌ", english: "Woe" },
        { arabic: "يَوْمَئِذٍ", english: "that Day" },
        { arabic: "لِّلْمُكَذِّبِينَ", english: "to the deniers" }
      ]
    },
    {
      number: 16,
      arabic: "أَلَمْ نُهْلِكِ الْأَوَّلِينَ",
      translation: "Did We not destroy the former peoples?",
      words: [
        { arabic: "أَلَمْ", english: "Did not" },
        { arabic: "نُهْلِكِ", english: "We destroy" },
        { arabic: "الْأَوَّلِينَ", english: "the former peoples" }
      ]
    },
    {
      number: 17,
      arabic: "ثُمَّ نُتْبِعُهُمُ الْآخِرِينَ",
      translation: "Then We will follow them with the later ones",
      words: [
        { arabic: "ثُمَّ", english: "Then" },
        { arabic: "نُتْبِعُهُمُ", english: "We will follow them" },
        { arabic: "الْآخِرِينَ", english: "with the later ones" }
      ]
    },
    {
      number: 18,
      arabic: "كَذَٰلِكَ نَفْعَلُ بِالْمُجْرِمِينَ",
      translation: "Thus do We deal with the criminals",
      words: [
        { arabic: "كَذَٰلِكَ", english: "Thus" },
        { arabic: "نَفْعَلُ", english: "do We deal" },
        { arabic: "بِالْمُجْرِمِينَ", english: "with the criminals" }
      ]
    },
    {
      number: 19,
      arabic: "وَيْلٌ يَوْمَئِذٍ لِّلْمُكَذِّبِينَ",
      translation: "Woe, that Day, to the deniers",
      words: [
        { arabic: "وَيْلٌ", english: "Woe" },
        { arabic: "يَوْمَئِذٍ", english: "that Day" },
        { arabic: "لِّلْمُكَذِّبِينَ", english: "to the deniers" }
      ]
    },
    {
      number: 20,
      arabic: "أَلَمْ نَخْلُقْكُمْ مِّن مَّاءٍ مَّهِينٍ",
      translation: "Did We not create you from a liquid disdained?",
      words: [
        { arabic: "أَلَمْ", english: "Did not" },
        { arabic: "نَخْلُقْكُمْ", english: "We create you" },
        { arabic: "مِّن", english: "from" },
        { arabic: "مَّاءٍ", english: "a liquid" },
        { arabic: "مَّهِينٍ", english: "disdained" }
      ]
    },
    {
      number: 21,
      arabic: "فَجَعَلْنَاهُ فِی قَرَارٍ مَّكِينٍ",
      translation: "And We placed it in a firm lodging",
      words: [
        { arabic: "فَجَعَلْنَاهُ", english: "And We placed it" },
        { arabic: "فِی", english: "in" },
        { arabic: "قَرَارٍ", english: "a lodging" },
        { arabic: "مَّكِينٍ", english: "firm" }
      ]
    },
    {
      number: 22,
      arabic: "إِلَیٰ قَدَرٍ مَّعْلُومٍ",
      translation: "For a known extent",
      words: [
        { arabic: "إِلَیٰ", english: "For" },
        { arabic: "قَدَرٍ", english: "an extent" },
        { arabic: "مَّعْلُومٍ", english: "known" }
      ]
    },
    {
      number: 23,
      arabic: "فَقَدَرْنَا فَنِعْمَ الْقَادِرُونَ",
      translation: "And We determined [it], and excellent [are We] to determine",
      words: [
        { arabic: "فَقَدَرْنَا", english: "And We determined" },
        { arabic: "فَنِعْمَ", english: "and excellent" },
        { arabic: "الْقَادِرُونَ", english: "are We to determine" }
      ]
    },
    {
      number: 24,
      arabic: "وَيْلٌ يَوْمَئِذٍ لِّلْمُكَذِّبِينَ",
      translation: "Woe, that Day, to the deniers",
      words: [
        { arabic: "وَيْلٌ", english: "Woe" },
        { arabic: "يَوْمَئِذٍ", english: "that Day" },
        { arabic: "لِّلْمُكَذِّبِينَ", english: "to the deniers" }
      ]
    },
    {
      number: 25,
      arabic: "أَلَمْ نَجْعَلِ الْأَرْضَ كِفَاتًا",
      translation: "Have We not made the earth a container",
      words: [
        { arabic: "أَلَمْ", english: "Have not" },
        { arabic: "نَجْعَلِ", english: "We made" },
        { arabic: "الْأَرْضَ", english: "the earth" },
        { arabic: "كِفَاتًا", english: "a container" }
      ]
    },
    {
      number: 26,
      arabic: "أَحْيَاءً وَأَمْوَاتًا",
      translation: "Of the living and the dead?",
      words: [
        { arabic: "أَحْيَاءً", english: "Of the living" },
        { arabic: "وَأَمْوَاتًا", english: "and the dead" }
      ]
    },
    {
      number: 27,
      arabic: "وَجَعَلْنَا فِيهَا رَوَاسِیَ شَامِخَاتٍ وَأَسْقَيْنَاكُمْ مَّاءً فُرَاتًا",
      translation: "And We placed therein lofty, firmly set mountains and have given you to drink sweet water",
      words: [
        { arabic: "وَجَعَلْنَا", english: "And We placed" },
        { arabic: "فِيهَا", english: "therein" },
        { arabic: "رَوَاسِیَ", english: "mountains" },
        { arabic: "شَامِخَاتٍ", english: "lofty, firmly set" },
        { arabic: "وَأَسْقَيْنَاكُمْ", english: "and have given you to drink" },
        { arabic: "مَّاءً", english: "water" },
        { arabic: "فُرَاتًا", english: "sweet" }
      ]
    },
    {
      number: 28,
      arabic: "وَيْلٌ يَوْمَئِذٍ لِّلْمُكَذِّبِينَ",
      translation: "Woe, that Day, to the deniers",
      words: [
        { arabic: "وَيْلٌ", english: "Woe" },
        { arabic: "يَوْمَئِذٍ", english: "that Day" },
        { arabic: "لِّلْمُكَذِّبِينَ", english: "to the deniers" }
      ]
    },
    {
      number: 29,
      arabic: "انطَلِقُوا إِلَیٰ مَا كُنتُمْ بِهِ تُكَذِّبُونَ",
      translation: "Proceed to that which you used to deny",
      words: [
        { arabic: "انطَلِقُوا", english: "Proceed" },
        { arabic: "إِلَیٰ", english: "to" },
        { arabic: "مَا", english: "that which" },
        { arabic: "كُنتُمْ", english: "you used to" },
        { arabic: "بِهِ", english: "it" },
        { arabic: "تُكَذِّبُونَ", english: "deny" }
      ]
    },
    {
      number: 30,
      arabic: "انطَلِقُوا إِلَیٰ ظِلٍّ ذِی ثَلَاثِ شُعَبٍ",
      translation: "Proceed to a shadow [of smoke] having three columns",
      words: [
        { arabic: "انطَلِقُوا", english: "Proceed" },
        { arabic: "إِلَیٰ", english: "to" },
        { arabic: "ظِلٍّ", english: "a shadow" },
        { arabic: "ذِی", english: "having" },
        { arabic: "ثَلَاثِ", english: "three" },
        { arabic: "شُعَبٍ", english: "columns" }
      ]
    },
    {
      number: 31,
      arabic: "لَّا ظَلِيلٍ وَلَا يُغْنِی مِنَ اللَّهَبِ",
      translation: "[But having] no cool shade and availing not against the flame",
      words: [
        { arabic: "لَّا", english: "No" },
        { arabic: "ظَلِيلٍ", english: "cool shade" },
        { arabic: "وَلَا", english: "and not" },
        { arabic: "يُغْنِی", english: "availing" },
        { arabic: "مِنَ", english: "against" },
        { arabic: "اللَّهَبِ", english: "the flame" }
      ]
    },
    {
      number: 32,
      arabic: "إِنَّهَا تَرْمِی بِشَرَرٍ كَالْقَصْرِ",
      translation: "Indeed, it throws sparks [as huge] as a fortress",
      words: [
        { arabic: "إِنَّهَا", english: "Indeed, it" },
        { arabic: "تَرْمِی", english: "throws" },
        { arabic: "بِشَرَرٍ", english: "sparks" },
        { arabic: "كَالْقَصْرِ", english: "as huge as a fortress" }
      ]
    },
    {
      number: 33,
      arabic: "كَأَنَّهُ جِمَالَتٌ صُفْرٌ",
      translation: "As if they were yellowish [black] camels",
      words: [
        { arabic: "كَأَنَّهُ", english: "As if they were" },
        { arabic: "جِمَالَتٌ", english: "camels" },
        { arabic: "صُفْرٌ", english: "yellowish black" }
      ]
    },
    {
      number: 34,
      arabic: "وَيْلٌ يَوْمَئِذٍ لِّلْمُكَذِّبِينَ",
      translation: "Woe, that Day, to the deniers",
      words: [
        { arabic: "وَيْلٌ", english: "Woe" },
        { arabic: "يَوْمَئِذٍ", english: "that Day" },
        { arabic: "لِّلْمُكَذِّبِينَ", english: "to the deniers" }
      ]
    },
    {
      number: 35,
      arabic: "هَٰذَا يَوْمُ لَا يَنطِقُونَ",
      translation: "This is a Day they will not speak",
      words: [
        { arabic: "هَٰذَا", english: "This is" },
        { arabic: "يَوْمُ", english: "a Day" },
        { arabic: "لَا", english: "not" },
        { arabic: "يَنطِقُونَ", english: "they will speak" }
      ]
    },
    {
      number: 36,
      arabic: "وَلَا يُؤْذَنُ لَهُمْ فَيَعْتَذِرُونَ",
      translation: "Nor will it be permitted for them to make an excuse",
      words: [
        { arabic: "وَلَا", english: "Nor will" },
        { arabic: "يُؤْذَنُ", english: "it be permitted" },
        { arabic: "لَهُمْ", english: "for them" },
        { arabic: "فَيَعْتَذِرُونَ", english: "to make an excuse" }
      ]
    },
    {
      number: 37,
      arabic: "وَيْلٌ يَوْمَئِذٍ لِّلْمُكَذِّبِينَ",
      translation: "Woe, that Day, to the deniers",
      words: [
        { arabic: "وَيْلٌ", english: "Woe" },
        { arabic: "يَوْمَئِذٍ", english: "that Day" },
        { arabic: "لِّلْمُكَذِّبِينَ", english: "to the deniers" }
      ]
    },
    {
      number: 38,
      arabic: "هَٰذَا يَوْمُ الْفَصْلِ ۖ جَمَعْنَاكُمْ وَالْأَوَّلِينَ",
      translation: "This is the Day of Judgement; We will have assembled you and the former peoples",
      words: [
        { arabic: "هَٰذَا", english: "This is" },
        { arabic: "يَوْمُ", english: "the Day" },
        { arabic: "الْفَصْلِ", english: "of Judgement" },
        { arabic: "جَمَعْنَاكُمْ", english: "We will have assembled you" },
        { arabic: "وَالْأَوَّلِينَ", english: "and the former peoples" }
      ]
    },
    {
      number: 39,
      arabic: "فَإِن كَانَ لَكُمْ كَيْدٌ فَكِيدُونِ",
      translation: "So if you have a plan, then plan against Me",
      words: [
        { arabic: "فَإِن", english: "So if" },
        { arabic: "كَانَ", english: "there was" },
        { arabic: "لَكُمْ", english: "for you" },
        { arabic: "كَيْدٌ", english: "a plan" },
        { arabic: "فَكِيدُونِ", english: "then plan against Me" }
      ]
    },
    {
      number: 40,
      arabic: "وَيْلٌ يَوْمَئِذٍ لِّلْمُكَذِّبِينَ",
      translation: "Woe, that Day, to the deniers",
      words: [
        { arabic: "وَيْلٌ", english: "Woe" },
        { arabic: "يَوْمَئِذٍ", english: "that Day" },
        { arabic: "لِّلْمُكَذِّبِينَ", english: "to the deniers" }
      ]
    },
    {
      number: 41,
      arabic: "إِنَّ الْمُتَّقِينَ فِی ظِلَالٍ وَعُيُونٍ",
      translation: "Indeed, the righteous will be among shades and springs",
      words: [
        { arabic: "إِنَّ", english: "Indeed" },
        { arabic: "الْمُتَّقِينَ", english: "the righteous" },
        { arabic: "فِی", english: "will be among" },
        { arabic: "ظِلَالٍ", english: "shades" },
        { arabic: "وَعُيُونٍ", english: "and springs" }
      ]
    },
    {
      number: 42,
      arabic: "وَفَوَاكِهَ مِمَّا يَشْتَهُونَ",
      translation: "And fruits from whatever they desire",
      words: [
        { arabic: "وَفَوَاكِهَ", english: "And fruits" },
        { arabic: "مِمَّا", english: "from whatever" },
        { arabic: "يَشْتَهُونَ", english: "they desire" }
      ]
    },
    {
      number: 43,
      arabic: "كُلُوا وَاشْرَبُوا هَنِيئًا بِمَا كُنتُمْ تَعْمَلُونَ",
      translation: "Eat and drink in satisfaction for what you used to do",
      words: [
        { arabic: "كُلُوا", english: "Eat" },
        { arabic: "وَاشْرَبُوا", english: "and drink" },
        { arabic: "هَنِيئًا", english: "in satisfaction" },
        { arabic: "بِمَا", english: "for what" },
        { arabic: "كُنتُمْ", english: "you used to" },
        { arabic: "تَعْمَلُونَ", english: "do" }
      ]
    },
    {
      number: 44,
      arabic: "إِنَّا كَذَٰلِكَ نَجْزِی الْمُحْسِنِينَ",
      translation: "Indeed, We thus reward the doers of good",
      words: [
        { arabic: "إِنَّا", english: "Indeed, We" },
        { arabic: "كَذَٰلِكَ", english: "thus" },
        { arabic: "نَجْزِی", english: "reward" },
        { arabic: "الْمُحْسِنِينَ", english: "the doers of good" }
      ]
    },
    {
      number: 45,
      arabic: "وَيْلٌ يَوْمَئِذٍ لِّلْمُكَذِّبِينَ",
      translation: "Woe, that Day, to the deniers",
      words: [
        { arabic: "وَيْلٌ", english: "Woe" },
        { arabic: "يَوْمَئِذٍ", english: "that Day" },
        { arabic: "لِّلْمُكَذِّبِينَ", english: "to the deniers" }
      ]
    },
    {
      number: 46,
      arabic: "كُلُوا وَتَمَتَّعُوا قَلِيلًا إِنَّكُمْ مُّجْرِمُونَ",
      translation: "[O disbelievers], eat and enjoy yourselves a little; indeed, you are criminals",
      words: [
        { arabic: "كُلُوا", english: "Eat" },
        { arabic: "وَتَمَتَّعُوا", english: "and enjoy yourselves" },
        { arabic: "قَلِيلًا", english: "a little" },
        { arabic: "إِنَّكُمْ", english: "indeed, you are" },
        { arabic: "مُّجْرِمُونَ", english: "criminals" }
      ]
    },
    {
      number: 47,
      arabic: "وَيْلٌ يَوْمَئِذٍ لِّلْمُكَذِّبِينَ",
      translation: "Woe, that Day, to the deniers",
      words: [
        { arabic: "وَيْلٌ", english: "Woe" },
        { arabic: "يَوْمَئِذٍ", english: "that Day" },
        { arabic: "لِّلْمُكَذِّبِينَ", english: "to the deniers" }
      ]
    },
    {
      number: 48,
      arabic: "وَإِذَا قِيلَ لَهُمُ ارْكَعُوا لَا يَرْكَعُونَ",
      translation: "And when it is said to them, 'Bow [in prayer],' they do not bow",
      words: [
        { arabic: "وَإِذَا", english: "And when" },
        { arabic: "قِيلَ", english: "it is said" },
        { arabic: "لَهُمُ", english: "to them" },
        { arabic: "ارْكَعُوا", english: "Bow in prayer" },
        { arabic: "لَا", english: "not" },
        { arabic: "يَرْكَعُونَ", english: "they do bow" }
      ]
    },
    {
      number: 49,
      arabic: "وَيْلٌ يَوْمَئِذٍ لِّلْمُكَذِّبِينَ",
      translation: "Woe, that Day, to the deniers",
      words: [
        { arabic: "وَيْلٌ", english: "Woe" },
        { arabic: "يَوْمَئِذٍ", english: "that Day" },
        { arabic: "لِّلْمُكَذِّبِينَ", english: "to the deniers" }
      ]
    },
    {
      number: 50,
      arabic: "فَبِأَیِّ حَدِيثٍ بَعْدَهُ يُؤْمِنُونَ",
      translation: "Then in what statement after the Qur'an will they believe?",
      words: [
        { arabic: "فَبِأَیِّ", english: "Then in what" },
        { arabic: "حَدِيثٍ", english: "statement" },
        { arabic: "بَعْدَهُ", english: "after it" },
        { arabic: "يُؤْمِنُونَ", english: "will they believe" }
      ]
    }
  ],
  videos: [
    {
      id: "4f8yTAWNiDM",
      title: "World's Most Beautiful Explanation of Surah Al-Mursalat",
      channel: "Nouman Ali Khan",
      description: "A comprehensive, full-length tafsir of the entire surah. Nouman Ali Khan walks through the rhetorical structure, explaining how the opening oaths about winds and angels build to the refrain 'Woe to the deniers.' Ideal as your first deep-dive into the surah's meaning.",
      tags: ["Full Tafsir", "Rhetorical Analysis", "Beginner-Friendly"],
      ayahsCovered: "1–50 (Full Surah)",
      scores: { authority: 5, relevance: 5, depth: 5, engagement: 5, clarity: 5 }
    },
    {
      id: "Iz0gXt4N_-o",
      title: "You Started From Something Humiliating — Surah Al-Mursalat",
      channel: "Nouman Ali Khan",
      description: "A focused talk on Ayahs 20–23 where Allah confronts human arrogance by reminding us we were created from a despised liquid. Nouman Ali Khan unpacks why this blunt reminder is placed right after the refrain 'Woe to the deniers' — connecting pride to denial. Short, punchy, and memorable.",
      tags: ["Human Origins", "Humility", "Short Talk"],
      ayahsCovered: "20–23",
      scores: { authority: 5, relevance: 5, depth: 4, engagement: 4, clarity: 5 }
    },
    {
      id: "m5ZREEq6XtQ",
      title: "Allah's Armies (The Horse. The Wind. The Angels) — Surah Al-Mursalat",
      channel: "Nouman Ali Khan",
      description: "Breaks down the opening verses (1–7) where Allah swears by the winds sent forth, the angels who deliver revelation, and the forces of nature — all as armies carrying out His will. Helps you understand the powerful imagery behind the surah's name.",
      tags: ["Opening Verses", "Imagery", "Angels & Wind"],
      ayahsCovered: "1–7",
      scores: { authority: 5, relevance: 5, depth: 4, engagement: 4, clarity: 5 }
    },
    {
      id: "lxeHOuI8lQY",
      title: "Why Your Excuses Won't Matter on Judgment Day — Surah Al-Mursalat",
      channel: "Nouman Ali Khan",
      description: "Covers the Day of Judgment passages (35–40) where the deniers are silenced — they cannot speak, cannot make excuses, and are challenged to scheme against Allah if they can. A sobering reminder about accountability.",
      tags: ["Judgment Day", "Accountability", "Motivation"],
      ayahsCovered: "35–40",
      scores: { authority: 5, relevance: 4, depth: 4, engagement: 4, clarity: 5 }
    },
    {
      id: "vqLej5MspKs",
      playlist: "PLYO6Oz7uwCSiIvS8fBim58larO1tRs9uI",
      title: "Quran Tafsir Series — Surah Mulk to Mursalat (Full Playlist)",
      channel: "Dr. Yasir Qadhi & Shaykh Sajjad Gul",
      description: "A comprehensive Quran tafsir playlist. The relevant episode covers Juz 29 surahs including Al-Mursalat, discussing themes, context of revelation, and practical lessons. Links to the full playlist so you can explore all 30 sessions.",
      tags: ["Full Tafsir Series", "Scholarly Duo", "Playlist"],
      ayahsCovered: "1–50 (Full Surah)",
      scores: { authority: 5, relevance: 4, depth: 5, engagement: 4, clarity: 5 }
    }
  ]
};
